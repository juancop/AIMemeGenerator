...
class TwStreamListener(tweepy.StreamListener):
    ...
    runtime = 10
    twitter_folder_path = os.getcwd()
    name = settings.TABLE_NAME   # Nombre del archivo que almacena el pandas 
    ...
    def __init__(self):
        ...
        try:
            ...
            #    self.mydb.commit()
            #self.mycursor.close()
            for root, dirs, files in os.walk(twitter_folder_path):
                if name in files:
                    downloaded_tweets = pd.read_csv(+"./"+name)
                else:
                    var_specs = dict(settings.TABLE_COLUMNS, settings.TABLE_DTYPES)
                    downloaded_tweets = pd.DataFrame(columns = settings.TABLE_COLUMNS, dtype = var_specs)
                # return os.path.join(root, name)
        except Exception as error:
            print("Error: no se puede cargar el archivo",error)
    ...
    def on_status(self, status):
        ...
        try:
            ...
            downloaded_tweets = downloaded_tweets.append(val, ignore_index =True)
    ...
    def disconnect(self):
        downloaded_tweets.to_csv("./" + name)
        #self.mydb.close()
        return print("Stop Streaming")
    
    def run(self):
        print("Start Streaming")
        self.myStream.filter(languages=["es"], track = settings.TRACK_WORDS,is_async=True)#,locations=[-6.38,49.87,1.77,55.81])
